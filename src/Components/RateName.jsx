...
function RateName({ slug }) {
    ...
    const CheckIfRatesNameContainsSlug = (slug) => {
      for (let index = 0; index < user?.rateNames?.length; index++) {
        if (user.rateNames[index].nameId === slug) { return true; }        
      }
      return false;
    }
    ...
    useEffect(() => {
        if (CheckIfRatesNameContainsSlug(slug)) {
    ...
    const handleButtonClick = async () => {
        if (!showInput) {
            ...
            try {
              const response = await fetch('http://localhost:5001/api/users/rateName', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Authorization': 'Bearer ' + Cookies.get('logged_in'),
                ...
                body: JSON.stringify({ nameId: slug, Rate: number}), // Convert body to JSON string
                });
